...
7. Pass your plugin's resolved `apiUrl` to `createPluginToolset(descriptor, { apiUrl })` (a string or a `() => string | undefined` resolver evaluated per tool call). Through 0.6.0-3 the standard `{name}_status` / `{name}_config` / `{name}_config_apply` tools always called `http://127.0.0.1:<defaultPort>` and ignored `plugins.entries.<id>.config.apiUrl`, so on an instance with custom service ports they hit the wrong service. The second argument is now required (0.6.0-4 shipped it optional), so an unchanged `createPluginToolset(descriptor)` call no longer compiles; pass `{ apiUrl }`. `apiUrl` itself stays optional inside the options object: `{}` (or an unset `apiUrl`) deliberately keeps the default-port URL, which is only correct when the service runs on its default port.
8. If you use `registerPromptContext` (or any other conversation hook), declare it in `package.json`: `"jeeves": { "conversationHooks": ["before_prompt_build"] }`. `jeeves install` / `jeeves update` grant `plugins.entries.<id>.hooks.allowConversationAccess: true` only to plugins that declare one; without the grant OpenClaw skips the hook. Add a test with `recordRegisteredHooks` + `validateConversationHooks` so a missing declaration fails your build.
...
| Reference templates `content/templates/` (`spec.md`, `spec-to-code-guide.md`, written to `{configRoot}/jeeves-core/templates/`), `TEMPLATES_DIR` | None in core. The templates now ship with jeeves-tools' jeeves-design skill. `jeeves install` writes no templates and `jeeves uninstall` no longer deletes `jeeves-core/templates/` |
...
| `createPluginToolset(descriptor, { apiUrl })`, `PluginToolsetOptions`, `pluginToolsetOptionsSchema`, `PluginApiUrlResolver` | The standard tools call the plugin's configured `apiUrl` (string or per-call resolver); `defaultPort` is only the fallback. The options argument is required (breaking) |
...
- The reference templates written by v0.x to `{configRoot}/jeeves-core/templates/` are no longer updated. `jeeves install` and `jeeves uninstall` leave them in place; the maintained copies ship with jeeves-tools' jeeves-design skill, and removing the old folder is the owner's call.
