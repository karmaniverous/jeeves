...
7. Pass your plugin's resolved `apiUrl` to `createPluginToolset(descriptor, { apiUrl })` (a string or a `() => string | undefined` resolver evaluated per tool call). Through 0.6.0-3 the standard `{name}_status` / `{name}_config` / `{name}_config_apply` tools always called `http://127.0.0.1:<defaultPort>` and ignored `plugins.entries.<id>.config.apiUrl`, so on an instance with custom service ports they hit the wrong service. The option is optional (omitted, the old default-port URL is used), so an unchanged call still compiles but keeps the bug.
...
| `createPluginToolset(descriptor, { apiUrl })`, `PluginToolsetOptions`, `pluginToolsetOptionsSchema`, `PluginApiUrlResolver` | The standard tools call the plugin's configured `apiUrl` (string or per-call resolver); `defaultPort` is only the fallback (backward compatible) |
